...
async function analyzeRepository() {
    ...
    try {
        ...
        // Only add to recent projects if analysis succeeded
        updateRecentProjects(repoRoot);
...
// Add/update recent projects in localStorage
function updateRecentProjects(repoPath) {
    if (!repoPath) return;
    let recent = [];
    try {
    ...
    recent = recent.filter(p => p.path !== repoPath);
    // Add to top
    recent.unshift({ path: repoPath, lastOpened: Date.now() });
    // Keep only last 10
